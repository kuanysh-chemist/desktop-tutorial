...
export function WeeklyLineChart({ data, color = '#0f4c4c', label = '%' }) {
  return (
    <ResponsiveContainer width="100%" height={200}>
      <LineChart data={data} margin={{ left: -10, right: 16, top: 8, bottom: 4 }}>
        <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
        <XAxis dataKey="week" tickFormatter={formatRu} tick={{ fontSize: 10 }} />
...
export function LessonBarChart({ data, color = '#0f4c4c', label = '%' }) {
  return (
    <ResponsiveContainer width="100%" height={200}>
      <BarChart data={data} margin={{ left: -10, right: 16, top: 8, bottom: 4 }}>
        <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
        <XAxis dataKey="date" tickFormatter={formatRu} tick={{ fontSize: 10 }} />
        <YAxis domain={[0, 100]} tick={{ fontSize: 10 }} />
        <Tooltip labelFormatter={formatRu} formatter={(v) => [`${v ?? 0}%`, label]} />
